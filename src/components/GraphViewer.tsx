...
export default function GraphViewer() {
    ...
    function loadRaw(raw: unknown) {
        ...
        setError(null);
        const data = fromGraphifyExport(raw as GraphifyExport);
        setStats(statsFor(raw as GraphifyExport));
    ...
    return (
        <div style={{ position: 'relative', width: '100vw', height: '100vh' }}>
            ...
            <div style={panelStyle}>
                ...
                {stats && (
                    <div style={{ fontSize: 12, marginTop: 4, lineHeight: 1.5 }}>
                        <div>{stats.nodeCount} nodes · {stats.linkCount} edges</div>
                        <div>{stats.communityCount} communities · {stats.hyperedgeCount} hyperedges</div>
            ...
            {selected && (
                ...
                <div style={{ ...panelStyle, top: 'auto', left: 'auto', bottom: 16, right: 16 }}>
                    <strong style={{ fontSize: 13 }}>{String(selected.name)}</strong>
                    <div style={{ fontSize: 12, opacity: 0.8 }}>type: {String(selected.type)}</div>
                    {typeof selected.group === 'number' ? (
                        <div style={{ fontSize: 12, opacity: 0.8 }}>community: {selected.group}</div>
                    ) : null}
                    {selected.filePath ? (
                        <div style={{ fontSize: 12, opacity: 0.8 }}>{String(selected.filePath)}</div>
                    ) : null}
                </div>
            )}
        </div>
